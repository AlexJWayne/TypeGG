 ...
 *
 * **Note:** In a boolean context, a [StringName] will evaluate to `false` if it is empty (`StringName("")`). Otherwise, a [StringName] will always evaluate to `true`. The `not` operator cannot be used. Instead, [method is_empty] should be used to check for empty [StringName]s.
 *
*/
...
xml_unescape(): string;
  ...
  connect<T extends SignalsOf<StringName>>(signal: T, method: SignalFunction<StringName[T]>): number;
